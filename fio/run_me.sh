...
for fio_test in $test_method ; do
  for bs in $block_size ; do
    for ((i=1;i<=$repeat;i+=1)) ; do
      for ((size=$fio_test_size,nw=1;nw<=$workers;nw*=2,size/=2)); do    
	grep --with-filename iops $fio_output_dir/fio_$fio_test-$i-$nw-$bs.out | sort -t "-" -k 2n | sed -r "s/.*fio_output\///" >> results/fio_result_`date +"%d-%m-%Y-%H-%M-%S"`
      done
    done
  done
done
...
echo " Results : results/fio_result_`date +"%d-%m-%Y-%H-%M"`-*"
echo " ================================================= "
